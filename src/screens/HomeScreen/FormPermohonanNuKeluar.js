...
const FormPermohonanNuKeluar = ({ navigation, route }) => {
    ...
    const [capturedPhotoSTNK, setCapturedPhotoSTNK] = useState(null);
    const cameraRefSTNK = useRef(null);
    ...
    const [isCameraOpenSTNK, setIsCameraOpenSTNK] = useState(false);
    ...
    const handleCaptureSTNK = async () => {
        if (cameraRefSTNK.current) {
            try {
                const photo = await cameraRefSTNK.current.takePictureAsync();
                console.log('Captured photo:', photo);
                setCapturedPhotoSTNK(photo.uri);
            } catch (error) {
                console.error('Error taking picture:', error);
            }
        }
                ...
                >
                    ...
                    <View style={styles.mainCard}>
                        ...
                        <View style={styles.formCard}>
                            ...
                            <TextInput
                                style={styles.inputAlamat}
                                placeholder="Masukkan Masa Berlaku Uji Terakhir"
                                value={masaBerlakuUji}
                                onChangeText={(text) => setMasaBerlakuUji(text)}
                            />
                        </View>
                        ...
                        <View style={styles.cameraCard}>
                            <Text style={styles.formTitle}>STNK</Text>
                            {hasCameraPermission === null ? (
                                <View />
                            ) : hasCameraPermission === false ? (
                                <Text>No access to camera</Text>
                            ...
                            <TouchableOpacity
                                style={styles.captureButton}
                                onPress={() => setIsCameraOpenSTNK(!isCameraOpenSTNK)}
                            >
                                <Text style={styles.captureButtonText}>
                                    {isCameraOpenSTNK ? 'Close Camera' : 'Open Camera'}
                                </Text>
                            </TouchableOpacity>
                            {isCameraOpenSTNK && (
                                <TouchableOpacity
                                    style={styles.captureButton}
                                    onPress={handleCaptureSTNK}
                                >
                                    <Text style={styles.captureButtonText}>Capture Photo Kartu Uji</Text>
                                </TouchableOpacity>
                            )}
...
export default FormPermohonanNuKeluar;